            ...
            CsceneRender(true)
{
    ...
    for (ii=0;ii<numPointsH;ii++) //precomputes vectors (horizontal)
    {
            ...
            wi=zNear*tan(ai);//signed distance from the ray i to the x sensor axis, in the zNear plane. 
            kH.push_back( (int) ((0.5 - wi/widthM)*widthP) );//pixel index associated to ray i. wi/widthM in [0.5,..,-0.5]
