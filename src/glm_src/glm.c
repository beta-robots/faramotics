...
glmFirstPass(GLMmodel* model, FILE* file) 
{
    ...
    numvertices = numnormals = numtexcoords = numtriangles = numlines = 0;
    while(fscanf(file, "%s", buf) != EOF) {
        switch(buf[0]) {
        case '#':               /* comment */
            /* eat up rest of line */
    ...
    model->numlines			= numlines;
		//printf("Loading model (1st pass)... %d vertices, %d normals, %d text. coords., %d triangles, %d lines\n", numvertices, numnormals, numtexcoords, numtriangles, numlines);
 ...
 */
static GLvoid
glmSecondPass(GLMmodel* model, FILE* file, float x, float y, float z) 
{
    GLuint  numvertices;        /* number of vertices in model */
    GLuint  numnormals;         /* number of normals in model */
    GLuint  numtexcoords;       /* number of texcoords in model */
  ...
  //printf("2nd pass done. there are %d lines\n", numlines);
...
glmDelete(GLMmodel* model)
{
    ...
    if (model->lines)     free(model->lines);
 ...
 */
GLMmodel* 
glmReadOBJ(const char* filename, float x, float y, float z)
{
    GLMmodel* model;
    FILE*   file;
    int i, j;
    ...
    if (!file) {
        __glmFatalError( "glmReadOBJ() failed: can't open data file \"%s\".",
			 filename);
	return NULL; //added by andreu corominas 20110729
    }
    ...
    //printf("allocating data: there are %d lines\n", model->numlines);exit(0);
    ...
    if (model->numlines) {
        model->lines = (GLint*)malloc(sizeof(GLint) *
					    2 * (model->numlines + 1));
    }
    ...
    //printf("start 2nd pass\n");
    glmSecondPass(model, file, x, y, z);
    //printf("end 2nd pass\n");

    /* facet normals are not in the file, we have to compute them anyway */
    //printf("on facet normals\n");
    glmFacetNormals(model);
    //printf("done facet normals\n");
...
glmDraw(GLMmodel* model, GLuint mode)
{
	...
	if(model->numlines > 0)
	{
		glBegin(GL_LINES);
			unsigned int l;
			for(l=0; l < model->numlines; l++)
			{
				//printf("line from v#%d to v#%d\n", model->lines[2*l], model->lines[2*l+1]);
				glVertex3f(model->vertices[3 * model->lines[2*l]], model->vertices[3 * model->lines[2*l] + 1], model->vertices[3 * model->lines[2*l] + 2]);
				glVertex3f(model->vertices[3 * model->lines[2*l+1]], model->vertices[3 * model->lines[2*l+1] + 1], model->vertices[3 * model->lines[2*l+1] + 2]);
			}
		glEnd();
		glDrawArrays(GL_LINES, 0, model->numlines*2);
	}
